...
class CmdListController(NonInteractiveController):
    ...
    async def run(self, context):
        ...
        for i, cmd in enumerate(tuple(self.builtin_cmds) + tuple(self.cmds)):
            ...
            # drop the snap specific environment variables. If the command
            # is `env`, we should let the user manage the environment
            command = shlex.split(desc)[0]
            path = shutil.which(command)
            if path is not None:
                if not path.startswith("/snap") and command != "env":
